...
@section('title', 'Tambah Transaksi Arisan')
@section('header', 'Tambah Transaksi Arisan')

@section('content')
<div class="max-w-md mx-auto md:max-w-full md:mx-0 bg-white rounded-xl shadow-xl overflow-hidden transform transition-all duration-300 hover:shadow-2xl">
    {{-- Header dengan gradient background --}}
    <div class="bg-gradient-to-r from-[#65764a] to-[#52603c] p-6 text-white">
        <div class="flex items-center space-x-3">
            <div class="bg-white/20 p-2 rounded-lg">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6m7-10a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                </svg>
            </div>
            <div>
                <h3 class="text-xl font-bold">Tambah Transaksi Arisan</h3>
                <p class="text-white/80 text-sm mt-1">Buat transaksi arisan baru untuk anggota</p>
            </div>
        </div>
    </div>
    ...
    <div class="p-6 md:p-8 bg-gradient-to-b from-slate-50 to-white">
        <form action="{{ route('transaksi-arisan.store') }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
                ...
                <div class="group md:col-span-1">
                    <label for="anggota_id" class="flex items-center space-x-2 text-sm font-semibold text-slate-700 mb-2">
                ...
                {{-- Total Setoran --}}
                <div class="group md:col-span-1">
                    <label for="total_setoran" class="flex items-center space-x-2 text-sm font-semibold text-slate-700 mb-2">
                        <svg class="w-4 h-4 text-[#65764a]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="relative">
                        <span class="absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-500 text-sm md:text-base">Rp</span>
                        <input type="number" name="total_setoran" id="total_setoran" required
                ...
                {{-- Status Pembayaran --}}
                <div class="group md:col-span-1">
                    <label for="status_sudah_lunas" class="flex items-center space-x-2 text-sm font-semibold text-slate-700 mb-2">
                        <svg class="w-4 h-4 text-[#65764a]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="group">
                ...
                <div class="flex flex-col sm:flex-row gap-3">
                    <label class="flex items-center p-4 border-2 border-slate-200 rounded-lg cursor-pointer hover:border-[#65764a]/50 transition-all duration-200 group-hover:border-slate-300">
                    ...
                    <label class="flex items-center p-4 border-2 border-slate-200 rounded-lg cursor-pointer hover:border-[#65764a]/50 transition-all duration-200 group-hover:border-slate-300">
                        <input type="radio" name="status_menang_arisan" value="0"
                               class="w-4 h-4 text-[#65764a] border-slate-300 focus:ring-[#65764a] focus:ring-2" checked>
    ...
    <div class="px-6 md:px-8 py-4 bg-slate-50 border-t border-slate-100">
        <p class="text-xs md:text-sm text-slate-500 text-center flex items-center justify-center space-x-1">
            ...
            <span>Transaksi akan dicatat setelah menekan tombol "Simpan Transaksi"</span>
        </p>
    </div>
</div>
@endsection
